...
abstract class AbstractReporting
{
    ...
    /**
     * Determine whether the application's default database connection is PostgreSQL.
     */
    ...
    public function getTimeInterval($startDate, $endDate, $period)
    {
        if ($period == 'auto') {
            ...
            if (! empty($intervals)) {
                return [
                    'group_column' => $this->isPostgres() ? 'EXTRACT(MONTH FROM created_at)' : 'MONTH(created_at)',
                    'intervals' => $intervals,
                ];
            }
            ...
            if (! empty($intervals)) {
                return [
                    'group_column' => $this->isPostgres() ? 'EXTRACT(WEEK FROM created_at)' : 'WEEK(created_at)',
                    'intervals' => $intervals,
                ];
            }
             ...
             */
            return [
                'group_column' => $this->isPostgres() ? 'EXTRACT(DOY FROM created_at)' : 'DAYOFYEAR(created_at)',
            ...
            if ($this->isPostgres()) {
                $pgFormatter = Str::replaceArray('?', ['YYYY', 'MM', 'DD'], $formatter);
                $groupColumn = "TO_CHAR(created_at, '".$pgFormatter."')";
            } else {
